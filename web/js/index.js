    ...
    $.ajax({
        ...
        success : function (data){
            for(var i=0;i<data.data.length;i++){
                ...
                $('.titlenews.ui-grid-b').append('<div class="ui-block-a">*</div><div class="ui-block-b">'+date+' : </div><div class="ui-block-c"><a href="showcontent?content=news&id_new='+data.data[i].id_new+'"title="'+data.data[i].title+'" rel="external">'+title+'</a></div>');
            }
            $.ajax({
                url : 'content',
                data : {
...
function event(){
    $.ajax({
        ...
        success : function (data){
            ...
            $('.event.ui-grid-a').append('<div class="ui-block-a"><a href="showcontent?content=event&id_eve='+data.data[0].id_eve+'" rel="external"><img src="'+image[0]+'" class="image"/></a></div><div class="ui-block-b"><p class="firstevent"><a href="showcontent?content=event&id_eve='+data.data[0].id_eve+'" title="'+data.data[0].title+'" rel="external">'+title+'</a></p><p class="detailfirstevent">'+detail+'</p></div>');
            for(var i=1;i<data.data.length;i++){
                var image = data.data[i].image.split(',');
                var title = data.data[i].title.substr(0, 86)+"..";
                var detail = data.data[i].detail.substr(0, 314);
                $('.subevent.ui-grid-a').append('<div class="ui-block-a"><a href="showcontent?content=event&id_eve='+data.data[i].id_eve+'" rel="external"><img src="'+image[0]+'" class="image"/></a></div><div class="ui-block-b"><p class="title"><a href="showcontent?content=event&id_eve='+data.data[i].id_eve+'"title="'+data.data[i].title+'" rel="external">'+title+'</a></p><p class="detail">'+detail+'</p></div>');
            }
    ...
    $.ajax({
        ...
        success : function (data){
            for(var i=0;i<data.data.length;i++){
                ...
                $('.knowledge.ui-grid-a').append('<div class="ui-block-a"><a href="showcontent?content=knowledge&id_kno='+data.data[i].id_kno+'" rel="external"><img src="'+image[0]+'" class="image"/></a></div><div class="ui-block-b"><p class="title"><a href="showcontent?content=knowledge&id_kno='+data.data[i].id_kno+'" title="'+data.data[i].title+'" rel="external">'+title+'</a></p><p class="detail">'+detail+'</p></div>');
            }
